...
function setup(){
 ...
 ammo.scale=0.05
  ...
  bench1.setCollider("rectangle",0,0,80,170)
...
function spawnZombies(){
  if (frameCount % 120 === 0){
    ...
    zombie.y = Math.round(random(10,windowHeight));
    zombie.addImage(zombieimage);
    zombie.scale=0.15
    zombie.velocityX = 3;
    zombie.setCollider("circle",0,0,240)
    zombie.debug=true;
    
    zombiegroup.add(zombie);

    if(zombiegroup.collide(woodgroup)||zombiegroup.collide(benchgroup)||zombiegroup.collide(obstaclegroup)){
      zombie.velocityX=0
